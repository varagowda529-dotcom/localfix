...
function ProblemRow({ data }: { data: ProblemCard }) {
  ...
  return (
    <Link href={`/problems/${problem.id}`} className="flex items-center gap-3 rounded-xl border border-slate-100 bg-white px-4 py-3 transition hover:border-brand-300 hover:shadow-sm">
...
function Section({ icon: Icon, title, items, emptyText, tint = "text-brand-500" }: { icon: typeof ClipboardList; title: string; items: ProblemCard[]; emptyText: string; tint?: string; }) {
  return (
    <section className="card p-5 sm:p-6">
      <div className="flex items-center justify-between">
        ...
        <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-extrabold text-slate-600">{items.length}</span>
      </div>
      {items.length === 0 ? (
        <p className="mt-4 rounded-xl bg-slate-50 px-4 py-6 text-center text-sm text-slate-400">{emptyText}</p>
      ) : (
...
function NotificationPanel({ items }: { items: Notification[] }) {
  return (
    <section className="card p-5 sm:p-6">
      ...
      {items.length === 0 ? (
        <p className="mt-4 rounded-xl bg-slate-50 px-4 py-6 text-center text-sm text-slate-400">Nothing yet — notifications about your reports will appear here.</p>
      ) : (
        <ul className="mt-4 space-y-2.5">
          {items.map((n) => (
            <li key={n.id}>
              <Link href={n.link ?? "/dashboard"} className={`flex items-start gap-3 rounded-xl border px-4 py-3 transition ${n.isRead ? "border-slate-100 bg-white hover:border-slate-200" : "border-brand-200 bg-brand-50/60 hover:border-brand-300"}`}>
...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>>; }) {
  ...
  // ADMIN DASHBOARD INTERCEPT
  if (user.isStaff) {
    const [stats, recentReports, pendingVerifications, overdueProblems] = await Promise.all([
      adminDashboardStats(),
      adminRecentReports(4),
      adminRecentVerifications(4),
      problemsByStatuses(["overdue"]),
    ]);

    return (
      <DashboardAdminClient
        user={{ ...user, firstName: user.fullName.split(" ")[0] }}
        stats={stats}
        recentReports={recentReports}
        pendingVerifications={pendingVerifications}
        overdueProblems={overdueProblems.slice(0, 4)}
  ...
  if (user.role === "government_employee") {
    ...
    const constituency = user.constituencyId ? all.find((l) => l.id === user.constituencyId) : undefined;
    ...
    <div className="container-site py-10">
      {sp.welcome === "1" && (
        <div className="form-banner-success mb-6 flex items-center gap-2.5">
          ...
          Welcome to LocalFix, {user.fullName.split(" ")[0]}! Your account is ready — this is your dashboard.
        </div>
      )}
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <span className="badge bg-slate-900 text-white ring-slate-900 ring-1">{roleLabel(user.role)}</span>
            {user.verificationStatus === "pending" ? (
              <span className="badge bg-amber-50 text-amber-700 ring-amber-200 ring-1"><Clock3 className="h-3.5 w-3.5" /> Verification Pending</span>
            ) : user.isVerified && user.role !== "citizen" ? (
              <span className="badge bg-emerald-50 text-emerald-700 ring-emerald-200 ring-1"><BadgeCheck className="h-3.5 w-3.5" /> Verified</span>
            ) : null}
          </div>
        </div>
      ...
      {user.verificationStatus === "pending" && (
        <div className="mt-6 flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
          ...
          <p className="text-sm leading-relaxed text-amber-800"><span className="font-bold">Your account verification is pending.</span> A moderator will review your account.</p>
        </div>
      )}
